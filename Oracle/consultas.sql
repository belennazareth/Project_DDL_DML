...
ALTER TABLE PEDIDOS MODIFY CONSTRAINT FK_CODART_PEDIDOS FOREIGN KEY (COD_ARTICULO) REFERENCES ARTICULOS (CODIGO) ON DELETE CASCADE;
...
SELECT avg(P.CANTIDAD) FROM TIENDAS T, PEDIDOS P WHERE T.NOMBRE=P.NOM_TIENDA;
...
SELECT V.NOMBRE, V.PAIS, A.NOMBRE, A.PRECIO_VENTA, T.NOMBRE FROM VENDEDORES V, ARTICULOS A, TIENDAS T
where v.codigo = (SELECT V.CODIGO FROM VENDEDORES V, ARTICULOS A WHERE V.CODIGO=A.COD_VENDEDOR ORDER BY A.PRECIO_VENTA ASC); 